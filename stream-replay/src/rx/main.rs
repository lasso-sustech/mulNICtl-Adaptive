...
fn main() {
    ...
    // print stuttering
    let stutter = &recv_data.stutter.ack_times;
    // create logger and write to file
    let mut logger = File::create( format!("logs/stuttering-{}.txt", port) ).unwrap();
    for ack_time in stutter {
        let message = format!("{}\n", ack_time);
