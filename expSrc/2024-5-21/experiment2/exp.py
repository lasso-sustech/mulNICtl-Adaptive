...
logger      = QosLogger(os.path.join(os.path.dirname(__file__), 'expRes_2.json'))
...
from util.solver import channelS2DMCSSolver

target_stream_name = ['proj1', 'proj2']

## TODO: get the initial MCS from the stream
sta1_sw_start = channelS2DMCSSolver( 'STA1', topo )
sta2_sw_start = channelS2DMCSSolver( 'STA2', topo )
...
for channel_switch_solver in channel_switch_solvers:
    channel_switch_solver.back_switch_threshold = 0.49
...
for test_idx in range(0, 10):
    ...
    ##########################: I (1)
    # if test_idx == 0:
    #     get_stream_by_name(streams, 'proj1').tx_parts = sta1_sw_start.mcs2transmission()
    #     get_stream_by_name(streams, 'proj2').tx_parts = sta2_sw_start.mcs2transmission()
    #     [ channel_solver.update_tx_parts(get_stream_by_name(streams, name).tx_parts) for channel_solver, name in zip(channel_solvers, target_stream_name) ]
    # else:
