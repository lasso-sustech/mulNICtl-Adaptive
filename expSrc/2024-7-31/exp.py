...
fileList = os.listdir('dpScript/2024-8-1/rtt/')
txtIndex = 0
...
topo        = construct_graph("./config/topo/2024-8-1.txt")
...
trans_manifests = {
    'file1': {
        ...
        'tx_parts'  : [0,0],
        'tos'       : 128,
        'channels'  : [constHead.CHANNEL0, constHead.CHANNEL0],
    },
    'file2': {
        'thru'      : thru,
        'file_type' : 'proj',
...
logger1 = open(f'dpScript/2024-8-1/TaskAll_{txtIndex}.jsonl', 'w')
...
def datatransfer():
    ...
    index = 0
    zeroIndex = 0
    logger1.write(f"interference: scenery 3; proj_thru {thru};file_thru {filethru}; stream num: {taskNum}"+ '\n')
    # logger1.write(f"inter_thru:{interthru}; inter num: {len(interList)}; inter_tos: {intertos}"+ '\n')
    logger2 = open(f'dpScript/2024-7-31/experiment1/Qos_{txtIndex}.jsonl', 'w')
    data_idx = 0
    portlist = ["6211@96","6212@96","6213@96"]
    while zeroIndex < 10:
        ...
        if type(data) == bytes:
            ...
            if len(datas) == 0:
                ...
                for port in portlist:
                    datas.pop(port, None)
                # datas.pop("6212@96", None)
                # datas.pop("6213@96", None)
                datasindex = 0
        ...
        plt.pause(0.01)
        index += 1
        plt.savefig(f"dpScript/2024-8-1/rtt/rtt_{txtIndex}.png")  
    plt.ioff()
    # plt.close()
...
def fileTransfer(targetIP,filepath):
    ...
    linkList = [links[0],links[2],links[4],links[6],links[4],links[6]]
...
for txtName in taskList:
    ...
    if os.path.exists(tempstutteringfileName):
        ...
        rt.Stutteringlist[port] = [NA_stuttering,Ada_stuttering,stuttering_gain]
rt.plot_rtt_stuttering(temptxtName,taskRttlist,rt.Stutteringlist,part = 0.4)
