...
fileList = os.listdir('dpScript/2024-7-26/experiment1/NoAda')
txtIndex = 0
...
inter_thru = 1
topo        = construct_graph("./config/topo/2024-7-26.txt")
...
for target_thru in thru_list:
    ...
    # # # packet Loss
    packetFileName = f"dpScript/2024-7-26/experiment1/PacketLoss/packetLoss{target_thru}_{txtIndex}.txt"
    packetFile = open(packetFileName,"w")
    packetFile.write(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime())+'\n')
    packetFile.write("interference : R*2+5+2F (5G)\n")
    ...
    NoAdapath = "dpScript/2024-7-26/experiment1/NoAda"
