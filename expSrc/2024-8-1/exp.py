...
trans_manifests = {
    ...
    'file3': {
        ...
        'link'      : links[8],
        'port'      : 6207,
        'links'     : [topo.get_link_ips(links[9]),topo.get_link_ips(links[8])],
        'tx_parts'  : [0,0],
        'tos'       : 128,
        'channels'  : [constHead.CHANNEL0,constHead.CHANNEL1],
    },
    'file4': {
        ...
        'link'      : links[10],
        'port'      : 6208,
        'links'     : [topo.get_link_ips(links[11]),topo.get_link_ips(links[10])],
        'tx_parts'  : [0,0],
        'tos'       : 128,
        'channels'  : [constHead.CHANNEL0,constHead.CHANNEL1],
    },
...
def datatransfer():
    ...
    logger2 = open(f'dpScript/2024-8-1/Qos_{txtIndex}.jsonl', 'w')
    data_idx = 0
    portlist = ["6209@96","6210@96"]
    while zeroIndex < 10:
        ...
        plt.pause(0.01)
        index += 1
        plt.savefig(f"dpScript/2024-8-1/rtt_{txtIndex}.png")  
    plt.ioff()
    # plt.close()
...
def fileTransfer(targetIP,filepath):
    ...
    linkList = [links[0],links[2],links[8],links[10]] #######################################
