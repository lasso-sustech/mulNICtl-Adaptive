...
fileList = os.listdir('dpScript/2024-8-2/rtt/')
txtIndex = 0
...
topo,links        = construct_graph("./config/topo/2024-8-2-2.txt")
# print("topo: ", topo)
...
logger1 = open(f'dpScript/2024-8-2/TaskAll_{txtIndex}.jsonl', 'w')
logger2 = open(f'dpScript/2024-8-2/Qos_{txtIndex}.jsonl', 'w')
...
def datatransfer():
    ...
    index = 0
    zeroIndex = 0
    logger1.write(f"No interference: scenery 3; proj_thru {thru};file_thru {filethru}; stream num: {taskNum}"+ '\n')
    ...
    while zeroIndex < 10:
        ...
        plt.pause(0.01)
        index += 1
        plt.savefig(f"dpScript/2024-8-2/rtt_{txtIndex}.png")  
    plt.ioff()
    # plt.close()
