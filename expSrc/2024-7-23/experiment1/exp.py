...
for target_thru in thru_list:
    inter_thru = int((400 - target_thru)/4)
    topo        = construct_graph("./config/topo/2024-7-23.txt")
    ...
    'file_inter2': {
            ...
            'link'      : links[3],
            'port'      : 6306,
            'links'     : [topo.get_link_ips(links[3])],
            'tx_parts'  : [1],
            'tos'       : 128,
        },
    # 'file_inter3': {
    ...
    logger = open(f'dpScript/2024-7-23/Adaptive/Adaptive-{target_thru}_cross_3x2x75M_149_gap{inter_gap}_{txtIndex}.jsonl', 'w')
    ...
    packetFileName = f"dpScript/2024-7-23/Adaptive/PacketLoss/packetLoss{target_thru}_{txtIndex}.txt"
    ...
    ctl.fileTransfer(topo, "192.168.3.72",  "dpScript/2024-7-23/Adaptive/NoAda", "../stream-replay/logs/rtt-6205*128.txt" , isSend=True)
    ...
    ctl.fileTransfer(topo, "192.168.3.72",  "dpScript/2024-7-23/Adaptive/Ada", "../stream-replay/logs/rtt-6205*128.txt" , isSend=True)
    txtIndex += 1
