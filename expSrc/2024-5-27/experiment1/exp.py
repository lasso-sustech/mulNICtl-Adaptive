...
topo        = construct_graph("./config/topo/2024-5-27.txt")
...
initial_throttle    = [51, 267]
# initial_throttle    = [120, 600]
for idx, target_name in enumerate(target_stream_name):
    get_stream_by_name(streams, target_name).throttle = initial_throttle[idx]

target_proj_stream_name = ['proj2']
channel_solvers = [ channelBalanceSolver() for _ in range(len(target_proj_stream_name)) ]
[ channel_solver.update_tx_parts(get_stream_by_name(streams, name).tx_parts) for channel_solver, name in zip(channel_solvers, target_proj_stream_name) ]
lighter = gb_state()
...
while True:
    ...
    if test_idx == 2:
        ...
        apply_control_to_stream_by_name(streams, thru_ctl)
    test_idx  += 1
    time.sleep(3) # wait for the system to be stable
    
    [ channel_solver.update( get_qos_by_name(qoses, name) ) for channel_solver, name in zip(channel_solvers, target_proj_stream_name) ]
