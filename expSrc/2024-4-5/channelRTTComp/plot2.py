import os
data_path = os.path.join(os.path.dirname(__file__), 'data2.json')
import json
...
def plot_data(datas):
    ...
    print(rtts_per_device)
    throughput = np.array(range(1, 20, 3)) * 5
    rtt_sum = rtts_per_device.sum(axis=0)
    ax.plot(throughput, rtt_sum[0:len(throughput)])
