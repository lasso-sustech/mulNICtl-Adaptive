...
def Stuttering_cal(filename,index=0,part = 0.45):
    ...
    if len(data)>1:
        ...
        for temp in Adadiff:
            if temp > 0.016:
                Ada_stuttering += temp
        # Ada_stuttering = sum(Adadiff[Adadiff>0])
...
def plot_json(filename):
    ...
    axs[3].plot(channel_colors["Index"],channel_colors["2.4G"],linewidth = 2, label="2.4G")
    axs[3].plot(channel_colors["Index"],channel_colors["5G"],linewidth = 2, label="5G")
...
def main():
    ...
    plot_rtt_stuttering(temptxtName,taskRttlist,Stutteringlist,part = 0.49)
