...
def result_plot_rttsecdict(log_file, save_path,rttsecdict):
    ...
    for idx,port in enumerate(task_list):
        ...
        axs[1].plot(IdxTimeSecrtt,temprttseclist, color = color_list[idx], label=port +" RTT")
        axs[1].plot(IdxTimeSecrtt,np.mean(temprttseclist)*np.ones(len(IdxTimeSecrtt)), label=port+'average RTT %.2f'%np.mean(temprttseclist))
